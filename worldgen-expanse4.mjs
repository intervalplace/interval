// Interval worldgen: THE EXPANSE, fourth founding (interval-expanse-v4).
//
...
export const GENERATOR_ID = 'interval-expanse-v4'
...
// ROOMS THAT ARE NOT IN A TOWN PLAN.
//
// groundKindAt learns a room's floor from the town DRAWING, and the Lantern
// stands alone out on the road with no drawing to read. So the one building
// in this world that is not in a plan says where it is, and says it the same
// way on every node, because it is derived from the genesis and nothing else.
...
function geographyHashE4(g0) {
  ...
  try {
    ...
    const h = E.sha256(Buffer.from('EXPANSE4-GEO-V2\n' + nodeSig + '\n' + terr.join(''))).toString('hex')
...
E.registerTerrain(GENERATOR_ID, {
  ...
  geographyHash: (g) => geographyHashE4(g),
  _isProbing: () => _probing,
})

const FOUNDER_KEY = '9e18ba7bc57d23737fefd36223acf7c173bbd26ffc3355d177f0a5fedfb220af'

export function makeExpanse4Genesis(genesisSeed, rulesHash, anchorMs = 0, W = 896, H = 512) {
  ...
  if (!_probing) g.geographyHash = geographyHashE4(g)
...
export function buildWorld(genesis) {
  ...
  { // the first tally, in Anchor's plaza
    ...
    seekT: for (let rad = 2; rad < 14; rad++) for (let dy = -rad; dy <= rad; dy++) for (let dx = -rad; dx <= rad; dx++) {
      ...
      put('tally-anchor', 'landmark', x, y, { kind: 'tally-half' }); break seekT
    }
  }
  { // the Ring, and Oberon
  ...
  const FURNITURE_FLOOR = {
    moor: 12.0,       // the great hole: heather, peat cuttings, boundary stones
    fens: 8.0,        // duckboards and eel traps
    greenwood: 10.0,
    crags: 10.0,
    downs: 13.0,      // already the best-furnished frontier country; nudge
    wilds: 3.0,       // stays sparse ON PURPOSE. A frontier reads as a
                      // frontier because there is nothing in it -- but zero
                      // is not sparse, it is unfinished.
  }
  const COUNTRY_KINDS = {
  ...
  const buildCountryThing = (x, y, kind, tag) => {
    ...
    switch (kind) {
      ...
      case 'charcoal': { // a burner's clamp and the ring of stumps around it
        ...
        put('char-' + tag + 'k', 'landmark', x + 1, y, { kind: 'standing-stone' })
        let n = 0
