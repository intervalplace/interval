...
// ---------- run it ----------
const html = readFileSync('window-writ.html', 'utf8')
...
const cam = seen.cam
ok(cam.isOrthographicCamera, 'the eye is orthographic: a tile is a tile wherever it is')
const fwd = new THREE.Vector3(0, 0, -1).applyQuaternion(cam.quaternion)
const aim = new THREE.Vector3(81, 1.5, 81).sub(cam.position).normalize()
ok(fwd.dot(aim) > 0.99, 'and it is pointed at the citizen (dot ' + fwd.dot(aim).toFixed(3) + ')')
...
const close = seen.main.children.filter((o) => o !== ground &&
  Math.hypot(o.position.x - 81, o.position.z - 81) < 14)
