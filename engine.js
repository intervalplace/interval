...
const STACKABLE = new Set(['iron-javelin', 'steel-javelin', 'star-javelin',
...
function validInput(state, input, ctx) {
  ...
  switch (input.type) {
    ...
    case 'special': {
      ...
      if (isRanged(p) && countItem(p.inventory, ammoOf(p)) < Math.max(1, w9.blows ?? 1)) return false;
