...
const uvOf = () => { const a = ground.geometry.attributes.uv, out = new Set()
  ...
  return out }
// §WHICH CELL IS UNDER THE CITIZEN. A window with a wide view has every kind of
// ground in the sample before the citizen ever steps on one, so "is a road cell
// anywhere in view" answers nothing. buildGround walks j from -R..R and i from
// -R..R, so the middle tile is the one at the middle of the buffer.
const cellHere = () => { const a = ground.geometry.attributes.uv
  const tiles = a.count / 4, span = Math.round(Math.sqrt(tiles)), R2 = (span - 1) / 2
  const k = (R2 * span + R2) * 4
  return a.getX(k).toFixed(2) + ',' + a.getY(k).toFixed(2) }
const heart = uvOf()
...
const heartCell = cellHere()
send({ type: 'state', state: at(30, 44), worldId: 'w' }); frames(8)
const fen = uvOf()
const fenCell = cellHere()
ok(fenCell !== heartCell, 'the fens do not look like the heartlands (' + heartCell + ' \u2192 ' + fenCell + ')')
...
const roadCell = cellHere()
ok(roadCell !== heartCell && roadCell !== fenCell,
   'a road under your feet looks like a road (' + roadCell + ')')
