...
relocated from the engine's own comments that have not yet been read
back against the code; they are binding and marked `[LIFTED]` until
someone checks them. `HISTORY.md` holds the record of what was tried
and repealed — it binds nothing and is hashed anyway, because a
constitution that can quietly drop its own history is not a record.

A node that hashes any of the three differently is playing a different
game and is refused at the door, exactly as before.
...
> **UNSETTLED (see HISTORY.md, §7cp/§7da).** This paragraph describes a
> recipe that no longer exists and a pack that no longer exists. The engine
> asks for thirty-two `planks` and eight `iron-ore`; `planks` is not in
> `STACKABLE`, so the cost is forty slots against a pack of twelve and
> **`raise_market` can never be satisfied**. The constitution will not state a
> cost until the engine states a reachable one.

The cost is most of a pack, so a stall cannot be carried with much else. It is
...
twelve stones, walk five tiles, empty the pack, mine
twelve more -- and the six thousand journeys out of the Wilds
...
every sale after it. A pack is twelve slots and a walk, and with
...
the pack is still twelve and is untouched by any of it. A citizen
...
The table below is GENERATED from the engine. Do not edit by hand — run
...
`altar`, `anvil`, `banner`, `bell`, `bellwork`, `brewpot`, `brimstone-vent`, `butt`, `campfire`, `cart`, `coal-rock`, `crier`, `dedication`, `deep-fish-spot`, `dummy`, `eel-spot`, `fence`, `ferry`, `fire`, `fishing-spot`, `fountain`, `furnace`, `gallows-oak`, `gibbet-shoal`, `gold-rock`, `guard`, `hearth`, `heartwood-tree`, `hedge`, `hoard`, `house`, `iron-rock`, `ironbark-tree`, `keeper`, `landmark`, `looking-glass`, `magic-rock`, `market`, `mother-lode`, `muck-heap`, `oak-tree`, `ossuary`, `plot`, `railing`, `rampart`, `rock`, `rockfall`, `salt-pan`, `sawpit`, `signpost`, `smith`, `smokerack`, `span`, `spanwork`, `stall`, `stamp`, `store`, `tollgate`, `tree`, `vault`, `wall`, `watchfire`, `waystone`, `well`.
<!-- END GENERATED: nodes -->
...
- `consign` → `{slots}` (v0.87); an array of one to twelve distinct
...
twelve to a trip. A mint that consumes its input is
...
**Forged** (`smith`, at an anvil). 54 recipes.
...
have both a geography and a vault that ignores it.
...
were redrawn; a name would collide across countries.

**Thirty-two vaults to a citizen**, which is what a citizen accumulates by
TRAVELLING, and §5's argument bites on it: state that grows with participation
eventually stops the world. There is no bound on KINDS beyond the item table
itself — a vault holding every item in the world is ninety-one keys, so the
five hundred and twelve this once carried could never fire, and a bound that
cannot fire reads as protection while being furniture.

**And eight thousand of any one kind, which is what a shelf holds** (§6al).
This is the half of the rule that makes goods CIRCULATE rather than merely sit
somewhere specific. Four hundred magic-stone is a star-plate (§5t), so a full
counter is twenty plates: enough to bank a long campaign in the Wilds, not
enough to mine for a year into one counter and never move. A career miner at
Cragfoot fills it and then has three choices — haul it, sell it at a stall, or
walk it somewhere else — and all three are the economy.

It is not a patience tax (§8). A cap cleared by making more trips would be
exactly that, and a script does not mind trips. This one is not cleared by
trips. It is cleared by the goods LEAVING.

**A deposit takes as much as fits** and the remainder stays in the pack, because
withdrawal already takes the least of what was asked, what is banked and what
will fit, and one question should not be answered two ways.

**The depth cap is a rule about depositing, not about the state.** A crossing
sums the shelves of a world that no longer exists and may seat more than a
deposit could add; destroying that excess would contradict a crossing carrying
a citizen whole. An over-full vault is lawful, refuses further deposits of that
kind, and drains back under the cap by being used.

**An emptied vault is deleted.** A citizen who cleared a counter does not carry
...
## 6m-vii. Food is a rate (v1.0)

**A food is a TOTAL and a RATE.** `healOf` is what it is worth, unchanged;
`FEED_RATE` is how fast it arrives. `eat` records a debt in hitpoints and pays
it down at the rate, so a cooked deep fish is ten at three an interval and a
mug of ale is four at one. Nothing is restored in the interval it is swallowed.

| rate | what | why |
|---|---|---|
| 3 | the deep catch | deep water is the master's water (§6ad); the fish out of it is the fastest mending in the world |
| 2 | the cooked | hot food off a fire, prepared for the fight it is for |
| 1 | the preserved and the brewed | salted fish, smoked eel, ale, broth — what a citizen carries because it stacks or keeps, sustaining the whole while |

**A debt, not a window**, because a rate that does not divide its total evenly
would lose or invent a hitpoint at the last interval. Cooked eel is seven at
two: two, two, two, one, done.

Eating restored the whole of it at once, so a fight was a contest in clicking
fish: damage landed and was undone at the same speed and the winner was
whoever brought the larger pack. §6m-ii already noticed the shape of this and
answered it with a rhythm — *the only thing stopping food from out-healing
damage* — which is a rate limit bolted onto a burst. The burst was the part
that was wrong.

**Nothing is rebalanced by this.** The rhythm already bounded how often a
citizen could eat, so the sustained healing was always `healOf` over the
rhythm. A window of the same length leaves that number exactly where it was.
The PEAK falls from six in one interval to one, and that is the whole change.

**More wins, and debts do not add.** Eating again takes the LARGER remaining
debt, or twelve slots of fish would be a burst again with extra steps. The rate
travels with whichever won: a rate without its food is nobody's food.

**Healing that would overflow the frame is still spent.** A citizen who eats at
full health has eaten; the alternative is a food that hangs unspent for ever
and mends at the exact moment of the next blow.

**A withering does not eat the food** (§7ck). The door is shut to this as it was
to the burst, and the debt is not spent while it is shut — the mending resumes
when it opens.

**The withered door is shut to this too** (§7ck). A rate that ignored it would
be the same healing arriving through another door.

What this leaves standing is the shape of a fight. Damage accumulates,
disengaging is the counter rather than swallowing, `anchor` has a job, and the
well (§6dd) is the only burst in the world — which is a PLACE, with a cooldown
...
|---|---|---|
| `barb` | hit 4 · every 2 · reach 1 · acc 8 | prowess 45 |
| `bare-blade` | hit 0 · every 2 · reach 1 · acc 0 · bare | prowess 30 |
| `bone-spear` | hit 0 · every 2 · reach 2 · acc 0 · desperate | prowess 50 |
...
When a hauler falls, the consignment no longer bursts into twelve piles
...
§11i left this open, and the reason it was open is that a heap of twelve
...
It is **a second container, not more pockets**. The pack is still twelve
...
2a. **A cart is taken only by somebody who could have been fought for it** —
   ...
   the road and wins a fight; a bystander carrying nothing lifts twelve
...
smiths it themselves pays for it twice.

> **UNSETTLED.** The table above and the hours below it were computed against a
> twenty-eight-slot consignment and a mastery of ninety-nine. The container is
> `INV_SLOTS` (twelve) and mastery is one hundred (§4b), so every `xp/trip`,
> `trips to 99` and `hours` figure in this section is stale. These are derived
> numbers, not rules: they must be re-derived from `haulXpFor` and the founded
> world's `genesis.haul`, not edited. Production very nearly cancels the multiplier. A self-sufficient
...
- **It pays no gold, and now the engine agrees.** Every draft that paid gold
  ...
  **A delivery is discharged, not sold.** §11a already says the cargo was
  ...
  meant to earn, and no village counter has to be able to afford twelve
  plates. XP is not rationed. Mine does not reduce yours.
- **It is now the largest goods sink in the world** — twelve to a trip
...
- **And the cargo's material value belongs to the thief.** A hauler who arrives
  earns experience; a hauler who is robbed hands somebody twelve slots of
...
> **Corrected before founding.** This paragraph continued: *because planks
> stack and logs do not, twenty-four slots becomes two.* `planks` is not in
> `STACKABLE` and never was, so the trade it describes was never available.
> A recipe payable in one trip is capped by the pack (§5t), and the stall's
> cost was set to ten planks and two iron-ore — twelve slots, the whole of
> it — rather than to a number the pack cannot hold.
...
So the toll is **one log**. It occupies a slot in a pack of twelve, it
...
# Appendix D. Drafted sections (DERIVED — NOT YET RATIFIED)

The engine cites these sections; the constitution did not have them. Each draft
below is the engine's own comment, lifted by `spec-stubs.mjs`, so that a gap
which was only a number in a test report is a paragraph somebody can read,
edit, cut, or promote. **Nothing in this appendix is law.** Promoting a draft
means moving it into the body above and deleting it from here.

<!-- BEGIN GENERATED: stubs -->
<!-- 140 sections drafted by spec-stubs.mjs from engine.js.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:6316` by `spec-stubs.mjs`. Amends §2b.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:2554` by `spec-stubs.mjs`. Amends §5j.

§5j-ii: what a landed blow used to pay across four ledgers, paid into one.

## 5k-ii. What The Berserker Gets For The Sixteen

> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:3270` by `spec-stubs.mjs`. Amends §5k.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:6377` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:6382` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:6400` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:6559` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:6598` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:6567` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:6555` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:14581` by `spec-stubs.mjs`. Amends §6af.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:14308` by `spec-stubs.mjs`. Amends §6af.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:14419` by `spec-stubs.mjs`. Amends §6af.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:3108` by `spec-stubs.mjs`. Amends §6af.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:2732` by `spec-stubs.mjs`. Amends §6af.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:2717` by `spec-stubs.mjs`. Amends §6af.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:10698` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:3772` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:3920` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:2561` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:4913` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:16248` by `spec-stubs.mjs`.

        const Tr = hitChance256(rLvl, stats.def, weaponOf(p)?.acc ?? 0, 0); // §6ap-ii

## 6aq. Repealed

> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:5044` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:14482` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:5239` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:3092` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:3333` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:3185` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:3792` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:7213` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:15166` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:3417` by `spec-stubs.mjs`.

§6bv: one incursion in thirty-two. They are events, not a farm.

## 6bv-ii. And What It Teaches Follows What Came Apart

> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:4056` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:2852` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:12051` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:11818` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:3182` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:4379` by `spec-stubs.mjs`.
...
## 6cz-iii. Twelve Hours

> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:3756` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:15972` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:10887` by `spec-stubs.mjs`. Amends §6m.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:14838` by `spec-stubs.mjs`. Amends §6m.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:5223` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:5080` by `spec-stubs.mjs`. Amends §6x.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:8005` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:3536` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:2942` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:3091` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:15094` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:2757` by `spec-stubs.mjs`.

§7by: identical, and cast in gold. That is the entire difference.

## 7ca. undrafted

> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:8427` by `spec-stubs.mjs`.

§7ca: what each blow of a flurry did, this interval only

## 7cb. Barrow-wight --

> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:3578` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:2873` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:16329` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:8341` by `spec-stubs.mjs`.

§7cw: whether the boat to the Lists knows them

## 7cx. The Siphon

> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:2787` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:4221` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:4884` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:8044` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:8094` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:2618` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:6030` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:8132` by `spec-stubs.mjs`.

§7dm: the hardest rung ever worked, per skill

## 7dn. The Ruin Nouns

> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:8105` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:4609` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:2992` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:5235` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:5773` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:15529` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:11732` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:7503` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:11436` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:7985` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:14170` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:9319` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:7998` by `spec-stubs.mjs`.
...
> **DERIVED — NOT YET RATIFIED.** Drafted from `engine.js:14286` by `spec-stubs.mjs`. Amends §21d.
