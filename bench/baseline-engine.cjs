...
function verifyPayload(sigHex, payloadBuf, pubHex) {
  try {
    ensureEdHash(); return ed.verify(Buffer.from(sigHex, 'hex'), payloadBuf, Buffer.from(pubHex, 'hex'));
  } catch {
    return false;
  }
}
...
function validateState(state) {
  ...
  const NODE_FIELDS = new Set(['type', 'x', 'y', 'depletedUntil', 'expiresAt', 'plantedAt', 'by', 'text', 'readyAt', 'brewKind', 'lastUsed']);
...
function validInput(state, input) {
  ...
  switch (input.type) {
    ...
    case 'build_brewpot': {
      ...
      if (Object.values(state.nodes).some(n => n.x === p.x && n.y === p.y)) return false;
      if (!Object.values(state.nodes).some(n => n.type === 'house' && adjacent(p, n))) return false;
      if (Object.values(state.nodes).filter(n => n.type === 'brewpot' && n.by === input.playerId).length >= bc.potCap) return false;
...
function surveyMarker(s, index, salt) {
  const g = s.genesis, anchor = spawnOf(g);
  const maxD = Math.max(anchor.x, g.worldW - anchor.x, anchor.y, g.worldH - anchor.y) || 1;
  const occupied = (x, y) => Object.values(s.nodes).some(n => n.x === x && n.y === y);
  const wsIds = Object.keys(s.nodes).filter(id => s.nodes[id].type === 'waystone').sort();
...
function nextState(state, inputs, _legacyBeacon) {
  ...
  for (const pid of order) {
    ...
    if (inp.type === 'move') {
      ...
      if (mi !== -1) {
        ...
        s.markers[mi] = surveyMarker(s, mi, 'claim:' + pid); // the point relocates
      }
      ...
      const free = !Object.values(s.nodes).some(n => n.x === p.x && n.y === p.y);
      const nearHouse = Object.values(s.nodes).some(n => n.type === 'house' && adjacent(p, n));
      const owned = Object.values(s.nodes).filter(n => n.type === 'brewpot' && n.by === pid).length;
      if (bc && free && nearHouse && owned < bc.potCap && countItem(p.inventory, 'logs') >= bc.buildLogs && countItem(p.inventory, 'ore') >= bc.buildOre) {
        ...
        s.nodes['brewpot-' + pid.slice(0, 8) + '-' + s.tick] = { type: 'brewpot', x: p.x, y: p.y, by: pid, lastUsed: s.tick };
      }
  ...
  for (const _pid in s.players) {
    ...
    if (inWilds(_p.x, _p.y) && claimFirst(s, 'wilds', _pid))
...
module.exports = {
  ...
  SLEEP_AFTER, isAwake, effLevel, validateState, validateGenesis, validateImports, validateInputShape, normalizeInput, slotOf, supportsWorldGenerator, minQuorumFor, maxByzantine, byzantineSafe, initCrypto, SKILLS, EQUIP_SLOTS, NODE_TYPES, INV_SLOTS, ITEMS, isValidName, cityRectOf, norwickRectOf, inCity, PRICES, inWilds, spawnOf, makeGenesis, newWorld, sameWorld, addPlayer, addNode, addMob, nextState, MOB_STATS, RECIPES, EQUIPPABLE,
};
